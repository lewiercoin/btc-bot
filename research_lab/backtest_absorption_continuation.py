...
from research_lab.setups import AbsorptionContinuationLong
from research_lab.setups.absorption_continuation import AbsorptionContinuationConfig
from settings import load_settings
from storage.repositories import fetch_funding_rates

PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_PATH = Path("research_lab/runs/absorption_continuation_backtest.json")


@dataclass(slots=True)
class AbsorptionDecisionRecord:
    timestamp: datetime
    regime: str
    ...
    price_near_ema50_atr: float | None = None
    price_near_equal_low: bool | None = None
...
class AbsorptionContinuationBacktestRunner(BacktestRunner):
    """Backtests absorption_continuation_long only, outside the live path."""

    def __init__(
        self,
        connection: sqlite3.Connection,
        *,
        setup: AbsorptionContinuationLong | None = None,
        **kwargs: Any,
    ) -> None:
        super().__init__(connection, **kwargs)
        self.setup = setup or AbsorptionContinuationLong()
    ...
    ) -> AbsorptionDecisionRecord:
        metrics = dict(getattr(evaluation, "metrics", {}) or {})
        record = AbsorptionDecisionRecord(
            ...
            pullback_depth_pct=_optional_float(metrics.get("pullback_depth_pct")),
            price_near_ema50_atr=_optional_float(metrics.get("price_near_ema50_atr")),
            price_near_equal_low=_optional_bool(metrics.get("price_near_equal_low")),
            cvd_bullish_divergence=bool(features.cvd_bullish_divergence),
...
def build_absorption_report(
    *,
    ...
    records: list[AbsorptionDecisionRecord],
    source_db_path: Path,
    start_date: str,
    end_date: str,
    ...
    absorption_confirmed = [record for record in closed if record.cvd_bullish_divergence or (record.tfi_60s or 0.0) >= 0.30]
    absorption_wins = [record for record in absorption_confirmed if record.pnl_r is not None and record.pnl_r > 0]
    return {
        ...
        "decision_summary": {
            ...
            "absorption_confirmation_hit_rate": _safe_div(len(absorption_wins), len(absorption_confirmed)),
        },
...
def write_absorption_markdown_report(report: dict[str, Any], output_path: Path) -> None:
    perf = report["performance"]
    summary = report["decision_summary"]
    lines = [
        "# Absorption Continuation Validation Report",
        "",
        ...
        f"- Absorption confirmation hit rate: {summary['absorption_confirmation_hit_rate']}",
        "",
        "## Per Regime",
        "",
...
def run_absorption_backtest(
    *,
    ...
    try:
        ...
        write_absorption_markdown_report(
            report,
            Path("research_lab/reports/absorption_continuation_validation_report.md"),
        )
        return report
    finally:
        conn.close()
...
def _top_rejection_reasons(records: list[AbsorptionDecisionRecord]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _record_to_json(record: AbsorptionDecisionRecord) -> dict[str, Any]:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest absorption_continuation_long research setup.")
