        ...
        with _connect(store_path) as conn:
            walkforward_rows = [
                ...
                for row in conn.execute(
                    "SELECT candidate_id, report_json, created_at_utc FROM walkforward_reports ORDER BY created_at_utc ASC"
                ).fetchall()
            ]
            recommendation_rows = [
                {
                ...
                for row in conn.execute(
                    "SELECT candidate_id, recommendation_json, created_at_utc FROM recommendations ORDER BY created_at_utc ASC"
                ).fetchall()
            ]
