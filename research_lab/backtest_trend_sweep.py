"""Backtest runner for Trend Sweep Specialist (sweep_reclaim family Variant 2).

Standalone script that:
1. Runs the standard sweep_reclaim signal engine
2. Applies Trend Sweep context pre-filters (regime=downtrend|uptrend only)
3. Overrides direction whitelist: counter-trend only (downtrend→LONG, uptrend→SHORT)
...
DEFAULT_JSON_PATH = DEFAULT_OUTPUT_DIR / "trend_sweep_specialist_results.json"
DEFAULT_MARKDOWN_PATH = DEFAULT_OUTPUT_DIR / "trend_sweep_specialist_validation_report.md"
DEFAULT_AUDIT_PATH = DEFAULT_OUTPUT_DIR / "TREND_SWEEP_SPECIALIST_AUDIT_PACKAGE.md"
...
class TrendSweepDecisionRecord:
    timestamp: str
    regime: str
...
# ---------------------------------------------------------------------------
# Trend Sweep Backtest Runner
# ---------------------------------------------------------------------------

class TrendSweepBacktestRunner(ResearchBacktestRunner):
    """Extends ResearchBacktestRunner with trend-sweep context pre-filters."""
    ...
    def __init__(
        ...
        *,
        ...
        replay_loader=None,
        fill_model=None,
        trend_config: TrendSweepConfig | None = None,
    ) -> None:
        super().__init__(connection, settings=settings, replay_loader=replay_loader, fill_model=fill_model)
        self.trend_config = trend_config or TrendSweepConfig()
        self.decision_records: list[TrendSweepDecisionRecord] = []
    ...
    def _build_engines(self):
        """Override to set counter-trend direction whitelist for trending regimes."""
        engines = super()._build_engines()
        feature_engine, regime_engine, context_engine, signal_engine, governance, risk_engine = engines

        # Override whitelist: counter-trend only in trending regimes
        new_whitelist = dict(signal_engine.config.regime_direction_whitelist)
        new_whitelist["downtrend"] = self.trend_config.downtrend_directions
        new_whitelist["uptrend"] = self.trend_config.uptrend_directions
    ...
    def run(self, config: BacktestConfig) -> BacktestResult:
        """Full replay with trend-sweep pre-filters injected into the signal resolution step."""
        ...
        for snapshot in replay_loader.iter_snapshots(
            ...
            record = TrendSweepDecisionRecord(
                timestamp=now.isoformat(),
                regime=regime.value,
                regime_accepted=False,
            )

            # --- Pre-filter 1: Regime must be trending ---
            trending, regime_reason = is_regime_trending(regime.value, self.trend_config)
            if not trending:
            ...
            # --- Pre-filter 2: Optional volatility check ---
            vol_ok, vol_reason = is_volatility_sufficient(
                float(features.atr_4h_norm), self.trend_config,
            )
            ...
            if candidate is not None:
                ...
                candidate.setup_type = f"trend_sweep_{candidate.direction.lower()}"
                candidate.reasons = [
                    f"setup_type=trend_sweep_specialist",
...
def compute_overlap(
    trend_sweep_trades: list[TradeLog],
    trial_00095_timestamps: list[str],
    cycle_seconds: int = 900,
) -> dict[str, Any]:
    """Compute trade overlap between Trend Sweep Specialist and trial-00095."""
    ...
    trend_sweep_cycles = []
    for trade in trend_sweep_trades:
        cycle = _floor_cycle(trade.opened_at.isoformat() if isinstance(trade.opened_at, datetime) else str(trade.opened_at))
        trend_sweep_cycles.append(cycle)

    trend_sweep_set = set(trend_sweep_cycles)
    overlap_cycles = trend_sweep_set & trial_cycles
    total = len(trend_sweep_set)
    overlap_count = len(overlap_cycles)
    overlap_rate = overlap_count / total if total > 0 else 0.0

    return {
        "trend_sweep_trades": len(trend_sweep_trades),
        "trend_sweep_unique_cycles": total,
...
def build_report(
    *,
    ...
    runner: TrendSweepBacktestRunner,
...
def write_validation_report(report: dict[str, Any], output_path: Path) -> None:
    ...
    if independence.get("status") == "not_computed":
        ...
        lines.extend([
            f"- Trend Sweep trades: {independence.get('trend_sweep_trades', 'N/A')}",
            f"- Trend Sweep unique cycles: {independence.get('trend_sweep_unique_cycles', 'N/A')}",
...
def run_trend_sweep_backtest(
    *,
    ...
    config = trend_config or TrendSweepConfig()
    conn = sqlite3.connect(str(source_db_path))
    conn.row_factory = sqlite3.Row
    try:
        settings = load_settings(project_root=PROJECT_ROOT, profile="research")
        runner = TrendSweepBacktestRunner(conn, settings=settings, trend_config=config)
        result = runner.run(
            BacktestConfig(
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest Trend Sweep Specialist (sweep_reclaim family variant 2)")
    ...
    parser.add_argument("--min-trend-cycles", type=int, default=0, help="Min cycles regime must persist before trading (0=disabled)")
    parser.add_argument("--volatility-min", type=float, default=0.006, help="Min atr_4h_norm for vol filter (default 0.006)")
    parser.add_argument("--enable-volatility-filter", action="store_true", help="Enable volatility minimum filter")
...
def main() -> int:
    args = _parse_args()
    config = TrendSweepConfig(
        min_trend_cycles=args.min_trend_cycles,
        volatility_atr_norm_min=args.volatility_min,
        volatility_filter_enabled=args.enable_volatility_filter,
    )
    run_trend_sweep_backtest(
