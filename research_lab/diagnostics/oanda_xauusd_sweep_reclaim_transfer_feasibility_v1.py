"""Research-only OANDA XAU_USD sweep/reclaim transfer diagnostic.

Implements OANDA_XAUUSD_SWEEP_RECLAIM_TRANSFER_FEASIBILITY_V1:
...
DEFAULT_REPORT_PATH = PROJECT_ROOT / "research_lab" / "reports" / "oanda_xauusd_sweep_reclaim_transfer_feasibility_v1.md"
DEFAULT_JSON_PATH = PROJECT_ROOT / "research_lab" / "reports" / "oanda_xauusd_sweep_reclaim_transfer_feasibility_v1.json"
DEFAULT_DATA_PATH = PROJECT_ROOT / "research_lab" / "analysis_output" / "oanda_xauusd_h1_candles_20240101_20260529.json"
...
def detect_sweep_state(
    ...
    for level in equal_lows:
        ...
        return SweepState(
            ...
            reclaimed=bool(reclaimed and wick_ok),
...
def build_event(
    ...
    if direction == "LONG":
        ...
        before_window = candles[state.detection_bar:entry_index]
    ...
    r_return = net_return / config.fixed_risk_pct if config.fixed_risk_pct else 0.0
                ...
                CohortEvent(
                    **{
                        **asdict(event),
                        ...
                        "r_return": (event.net_return_pct - delta) / config.fixed_risk_pct,
...
def data_quality(candles: list[Candle]) -> dict[str, Any]:
    ...
    return {
        ...
        "data_gate": "PASS" if len(candles) >= 10000 and bad_ohlc == 0 else "FAIL",
    }
...
def evaluate_gates(cohort_results: dict[str, dict[str, Any]], folds: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if main_median_net is None or main_median_net <= 0:
        stop_reasons.append("median_net_return_lte_0_at_0_05pct_cost")
...
def fetch_oanda_candles(credentials: OandaCredentials, config: DiagnosticConfig) -> list[Candle]:
    ...
    while cursor < end and request_count < 100:
        ...
        next_cursor = last_time + timedelta(hours=1)
...
def run_diagnostic(
    ...
    payload = {
        "diagnostic": "OANDA_XAUUSD_SWEEP_RECLAIM_TRANSFER_FEASIBILITY_V1",
...
def write_report(path: Path, payload: dict[str, Any], json_path: Path | None = None) -> None:
    ...
    lines: list[str] = [
        ...
        f"- Main ER proxy: `{_fmt(main['er'])}`",
        f"- Main PF proxy: `{_fmt(main['profit_factor'])}`",
        f"- Main win rate: `{_pct(main['win_rate'])}`",
        f"- Main median net return at 0.05% cost: `{_pct(main['median_net_return_pct'], 4)}`",
