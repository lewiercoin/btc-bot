#!/usr/bin/env python3
"""SOL asset-specific coarse optimization around frozen trial-00095.

Research Lab only. This milestone asks whether SOL improves with a small,
...
DEFAULT_REPORT = Path("docs/analysis/SOL_ASSET_SPECIFIC_OPTIMIZATION_2026-05-20.md")
DEFAULT_CACHE = Path("research_lab/snapshots/sol_asset_specific_optimization_v1_cache.json")
SYMBOL = "SOLUSDT"
...
def build_predeclared_grid() -> tuple[SolOptVariant, ...]:
    """Return the fixed SOL coarse grid.

    The grid deliberately changes only the high-leverage sweep-depth threshold.
    Other trial-00095 parameters remain frozen for this first SOL pass.
    """

    variants: list[SolOptVariant] = []
    for min_depth in (0.0055, 0.00649, 0.0075):
        suffix = f"D{min_depth:.5f}"
        variants.append(
            SolOptVariant(
                variant_id=f"SOL_OPT_{suffix}",
                overrides={
...
def baseline_variant(trial_params: dict[str, Any]) -> SolOptVariant:
    return SolOptVariant(
        variant_id="SOL_BASELINE_FROZEN_TRIAL_00095",
        overrides={
...
def train_passes(metrics: dict[str, Any], gates: SolOptGates) -> bool:
    return (
...
def builder_verdict(gates: dict[str, dict[str, Any]], *, selected_variant_id: str | None) -> str:
    if selected_variant_id is None:
        return "SOL_OPTIMIZATION_FAILED_NO_TRAIN_CANDIDATE"
    if all(item["pass"] for item in gates.values()):
        return "SOL_ASSET_SPECIFIC_CANDIDATE_FOR_AUDIT"
    return "SOL_OPTIMIZATION_NO_PROMOTION"
...
def evaluate_variant(
    *,
    ...
    settings = build_sol_settings(base_settings, trial_params, variant.overrides)
...
def run_analysis(
    *,
    ...
    try:
        ...
        baseline_settings = build_sol_settings(base_settings, trial_params, baseline.overrides)
        ...
        if selected is not None:
            selected_settings = build_sol_settings(base_settings, trial_params, selected.variant.overrides)
...
def load_cache(cache_path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(f"Invalid SOL optimization cache payload: {cache_path}")
    return payload
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-db", type=Path, default=DEFAULT_SOL_DB)
