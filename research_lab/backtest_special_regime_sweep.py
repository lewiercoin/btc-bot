"""Backtest runner for Special Regime Sweep Specialist (sweep_reclaim family Variant 3).

Standalone script that:
1. Runs the standard sweep_reclaim signal engine
2. Applies Special Regime context pre-filter (crowded_leverage + post_liquidation only)
3. Overrides direction whitelist: LONG only (SHORT dropped per V1+V2 evidence)
...
This is the FINAL regime test. If V3 fails (3/3), regime-based family expansion
is conclusively not viable.
"""
...
DEFAULT_JSON_PATH = DEFAULT_OUTPUT_DIR / "special_regime_sweep_specialist_results.json"
DEFAULT_MARKDOWN_PATH = DEFAULT_OUTPUT_DIR / "special_regime_sweep_specialist_validation_report.md"
DEFAULT_AUDIT_PATH = DEFAULT_OUTPUT_DIR / "SPECIAL_REGIME_SWEEP_SPECIALIST_AUDIT_PACKAGE.md"
...
class SpecialRegimeDecisionRecord:
    timestamp: str
    regime: str
...
# ---------------------------------------------------------------------------
# Special Regime Sweep Backtest Runner
# ---------------------------------------------------------------------------

class SpecialRegimeSweepBacktestRunner(ResearchBacktestRunner):
    """Extends ResearchBacktestRunner with special-regime context pre-filters."""
    ...
    def __init__(
        ...
        *,
        ...
        replay_loader=None,
        fill_model=None,
        special_config: SpecialRegimeSweepConfig | None = None,
    ) -> None:
        super().__init__(connection, settings=settings, replay_loader=replay_loader, fill_model=fill_model)
        self.special_config = special_config or SpecialRegimeSweepConfig()
        self.decision_records: list[SpecialRegimeDecisionRecord] = []
        ...
        # Per-regime cycle counters (for understanding regime distribution)
        self.regime_cycle_counts: dict[str, int] = {}

    def _build_engines(self):
        """Override to set LONG-only direction whitelist for special regimes."""
        engines = super()._build_engines()
        feature_engine, regime_engine, context_engine, signal_engine, governance, risk_engine = engines

        # Override whitelist: LONG only in special regimes
        directions = get_special_directions(self.special_config)
        new_whitelist = dict(signal_engine.config.regime_direction_whitelist)
        for regime in self.special_config.allowed_regimes:
            new_whitelist[regime] = directions
    ...
    def run(self, config: BacktestConfig) -> BacktestResult:
        """Full replay with special-regime pre-filters."""
        ...
        for snapshot in replay_loader.iter_snapshots(
            ...
            record = SpecialRegimeDecisionRecord(
                timestamp=now.isoformat(),
                regime=regime_key,
                regime_accepted=False,
            )

            # --- Pre-filter: Regime must be special ---
            special, regime_reason = is_regime_special(regime_key, self.special_config)
            if not special:
                record.rejection_reasons.append(regime_reason)
            ...
            if candidate is not None:
                ...
                candidate.setup_type = f"special_regime_sweep_{candidate.direction.lower()}"
                candidate.reasons = [
                    f"setup_type=special_regime_sweep_specialist",
                    f"regime={regime_key}",
                    *candidate.reasons,
                ]
...
def compute_overlap(
    special_trades: list[TradeLog],
    trial_00095_timestamps: list[str],
    cycle_seconds: int = 900,
) -> dict[str, Any]:
    """Compute trade overlap between Special Regime Sweep and trial-00095."""
    ...
    special_set = set(special_cycles)
    overlap_cycles = special_set & trial_cycles
    total = len(special_set)
    overlap_count = len(overlap_cycles)
    overlap_rate = overlap_count / total if total > 0 else 0.0

    return {
        "special_regime_trades": len(special_trades),
        "special_regime_unique_cycles": total,
...
def build_report(
    *,
    ...
    runner: SpecialRegimeSweepBacktestRunner,
    config: SpecialRegimeSweepConfig,
    source_db_path: str,
    start_date: str,
    end_date: str,
...
def write_validation_report(report: dict[str, Any], output_path: Path) -> None:
    ...
    regime_dist = report.get("regime_cycle_distribution", {})
    ...
    total_cycles = sum(regime_dist.values()) if regime_dist else 1
    ...
    if independence.get("status") == "not_computed":
        ...
        lines.extend([
            f"- Special Regime trades: {independence.get('special_regime_trades', 'N/A')}",
            f"- Special Regime unique cycles: {independence.get('special_regime_unique_cycles', 'N/A')}",
...
def write_audit_package(report: dict[str, Any], output_path: Path) -> None:
    ...
    lines.extend([
        "",
        "### Cross-Variant Summary (V1 + V2 + V3)",
        "",
        "| Variant | Context | Direction | Trades | ER | Finding |",
        "|---|---|---|---:|---:|---|",
        "| V1 | Normal | LONG | 16 | 0.02 | Zero edge |",
        "| V1 | Normal | SHORT | 5 | -0.92 | Destructive |",
        "| V2 | Downtrend | LONG | 127 | 0.76 | Moderate (overlaps trial-00095) |",
        "| V2 | Uptrend | SHORT | 32 | 0.09 | Zero edge |",
    ])
    # V3 rows added dynamically
    for r, row in sorted(report.get("per_regime", {}).items()):
        for d, drow in sorted(report.get("per_direction", {}).items()):
            lines.append(f"| V3 | {r} | {d} | {drow['trades_count']} | {drow['expectancy_r']} | V3 result |")
            break  # only one direction (LONG)
...
def run_special_regime_backtest(
    *,
    source_db_path: Path,
    ...
    config = special_config or SpecialRegimeSweepConfig()
    conn = sqlite3.connect(str(source_db_path))
    conn.row_factory = sqlite3.Row
    try:
        settings = load_settings(project_root=PROJECT_ROOT, profile="research")
        runner = SpecialRegimeSweepBacktestRunner(conn, settings=settings, special_config=config)
        result = runner.run(
            BacktestConfig(
        ...
        print(f"Regime distribution: {runner.regime_cycle_counts}")
        return report
    finally:
        conn.close()
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest Special Regime Sweep Specialist (sweep_reclaim family variant 3)")
    ...
    parser.add_argument("--min-regime-cycles", type=int, default=0, help="Min cycles regime must persist before trading (0=disabled)")
...
def main() -> int:
    args = _parse_args()
    config = SpecialRegimeSweepConfig(
        min_regime_cycles=args.min_regime_cycles,
    )
    run_special_regime_backtest(
