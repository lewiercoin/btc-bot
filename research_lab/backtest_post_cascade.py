...
from research_lab.setups import PostCascadeMomentumLong, PostCascadeMomentumShort
from settings import load_settings
from storage.repositories import fetch_funding_rates

PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_PATH = Path("research_lab/runs/post_cascade_backtest.json")
DEFAULT_MARKDOWN_PATH = Path("research_lab/reports/post_cascade_validation_report.md")


@dataclass(slots=True)
class PostCascadeDecisionRecord:
    timestamp: datetime
    regime: str
    ...
    cascade_direction: str | None = None
    cascade_direction_ratio: float | None = None
    force_orders_lookback: int = 0
...
class PostCascadeBacktestRunner(BacktestRunner):
    """Backtests post_cascade_momentum long/short only, outside the live path."""

    def __init__(self, connection: sqlite3.Connection, **kwargs: Any) -> None:
        super().__init__(connection, **kwargs)
        self.long_setup = PostCascadeMomentumLong()
        self.short_setup = PostCascadeMomentumShort()
        self.decision_records: list[PostCascadeDecisionRecord] = []
        self._records_by_object_id: dict[int, PostCascadeDecisionRecord] = {}
        self._records_by_signal_id: dict[str, PostCascadeDecisionRecord] = {}
    ...
    def run(self, config: BacktestConfig) -> BacktestResult:
        ...
        for snapshot in replay_loader.iter_snapshots(
            ...
            self._attach_force_order_history(snapshot=snapshot, symbol=symbol, now=now)
    ...
    def _attach_force_order_history(self, *, snapshot: Any, symbol: str, now: datetime) -> None:
        start = now - timedelta(minutes=self.long_setup.setup_config.force_lookback_minutes)
        rows = self.connection.execute(
            ...
            """,
            (symbol, start.isoformat(), now.isoformat()),
        ).fetchall()
        snapshot.source_meta["research_force_orders_lookback"] = [
            {
    ...
    def _generate_candidate(self, *, snapshot: Any, features: Features, regime: RegimeState) -> SignalCandidate | None:
        evaluations = [
            (
                self.long_setup,
                self.long_setup.evaluate_structure(features=features, snapshot=snapshot, regime=regime, config=self.settings.strategy),
                self.long_setup.generate_signal_candidate(features=features, snapshot=snapshot, regime=regime, config=self.settings.strategy),
            ),
            (
                self.short_setup,
                self.short_setup.evaluate_structure(features=features, snapshot=snapshot, regime=regime, config=self.settings.strategy),
                self.short_setup.generate_signal_candidate(features=features, snapshot=snapshot, regime=regime, config=self.settings.strategy),
            ),
        ]
        candidates = [candidate for _setup, _evaluation, candidate in evaluations if candidate is not None]
        candidate = max(candidates, key=lambda item: item.confluence_score) if candidates else None
        record = self._record_decision(features=features, regime=regime, evaluations=evaluations, candidate=candidate)
        if candidate is not None:
    ...
    def _record_decision(
        ...
        *,
        features: Features,
        regime: RegimeState,
        evaluations: list[tuple[Any, Any, SignalCandidate | None]],
        candidate: SignalCandidate | None,
    ) -> PostCascadeDecisionRecord:
        reasons: list[str] = []
        metrics: dict[str, Any] = {}
        for setup, evaluation, generated in evaluations:
            if generated is None:
                reasons.extend(f"{setup.get_setup_type()}:{reason}" for reason in list(evaluation.reasons))
            if generated is not None:
                metrics = dict(evaluation.metrics)
        if not metrics and evaluations:
            metrics = dict(evaluations[0][1].metrics)
        record = PostCascadeDecisionRecord(
            ...
            setup_type=None if candidate is None else candidate.setup_type,
            ...
            cascade_direction=metrics.get("cascade_direction"),
            cascade_direction_ratio=_optional_float(metrics.get("cascade_direction_ratio")),
            force_orders_lookback=int(metrics.get("force_orders_count", 0) or 0),
    ...
    def _register_governance_decision(self, candidate: SignalCandidate, decision: Any) -> None:
        ...
        if not bool(getattr(decision, "approved", False)):
            ...
            record.rejection_reasons.append(str(notes[0]) if notes else "governance_rejected")
    ...
    def _register_risk_decision(self, signal_id: str, decision: Any) -> None:
        ...
        record.rejection_reasons.append(str(reason) if reason is not None else "risk_rejected")
...
def build_post_cascade_report(
    *,
    result: BacktestResult,
    records: list[PostCascadeDecisionRecord],
    source_db_path: Path,
    start_date: str,
    end_date: str,
    ...
    continuation = [record for record in closed if record.pnl_r is not None and record.pnl_r > 0]
    return {
        ...
        "per_regime": _performance_by_regime(trades, initial_equity=initial_equity),
        "per_direction": _performance_by_direction(trades, initial_equity=initial_equity),
        "decision_summary": {
            "cycles": len(records),
            "post_liquidation_cycles": sum(1 for record in records if record.regime == "post_liquidation"),
            "candidates": len(generated),
            "closed_trades": len(closed),
            "cascade_continuation_rate": _safe_div(len(continuation), len(closed)),
...
def write_markdown_report(report: dict[str, Any], output_path: Path) -> None:
    ...
    lines = [
        "# Post Cascade Momentum Validation Report",
        "",
        ...
        f"- Post-liquidation cycles: {summary['post_liquidation_cycles']}",
        f"- Candidates: {summary['candidates']}",
        f"- Closed trades: {summary['closed_trades']}",
        f"- Cascade continuation rate: {summary['cascade_continuation_rate']}",
        "",
        "## Per Regime",
        "",
    ...
    lines.extend(["", "## Per Direction", "", "| Direction | Trades | ER | PF | DD |", "|---|---:|---:|---:|---:|"])
    for direction, row in sorted(report["per_direction"].items()):
        lines.append(f"| {direction} | {row['trades_count']} | {row['expectancy_r']} | {row['profit_factor']} | {row['max_drawdown_pct']} |")
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def run_post_cascade_backtest(
    *,
    source_db_path: Path,
    start_date: str,
    end_date: str,
    ...
    try:
        ...
        runner = PostCascadeBacktestRunner(conn, settings=settings)
        result = runner.run(
            BacktestConfig(start_date=start_date, end_date=end_date, symbol=settings.strategy.symbol, initial_equity=initial_equity)
        )
        report = build_post_cascade_report(
            result=result,
        ...
        write_markdown_report(report, DEFAULT_MARKDOWN_PATH)
        return report
    finally:
        conn.close()
...
def _performance_by_regime(trades: list[TradeLog], *, initial_equity: float) -> dict[str, dict[str, Any]]:
    ...
    return {regime: _dataclass_to_jsonable(summarize([trade for trade in trades if str(trade.regime) == regime], initial_equity=initial_equity)) for regime in regimes}


def _performance_by_direction(trades: list[TradeLog], *, initial_equity: float) -> dict[str, dict[str, Any]]:
    from backtest.performance import summarize

    directions = sorted({str(trade.direction) for trade in trades})
    return {direction: _dataclass_to_jsonable(summarize([trade for trade in trades if str(trade.direction) == direction], initial_equity=initial_equity)) for direction in directions}


def _top_rejection_reasons(records: list[PostCascadeDecisionRecord]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _record_to_json(record: PostCascadeDecisionRecord) -> dict[str, Any]:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest post_cascade_momentum research setup.")
