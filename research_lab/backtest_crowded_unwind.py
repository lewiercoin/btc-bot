...
from research_lab.setups import CrowdedUnwindLong, CrowdedUnwindShort
from settings import load_settings
from storage.repositories import fetch_funding_rates

PROJECT_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_OUTPUT_PATH = Path("research_lab/runs/crowded_unwind_backtest.json")
DEFAULT_MARKDOWN_PATH = Path("research_lab/reports/crowded_unwind_validation_report.md")


@dataclass(slots=True)
class CrowdedDecisionRecord:
    timestamp: datetime
    regime: str
    ...
    funding_pct_60d: float | None = None
    funding_8h: float | None = None
    oi_zscore_60d: float | None = None
    oi_delta_pct: float | None = None
    force_order_rate_60s: float | None = None
    force_order_spike: bool | None = None
...
class CrowdedUnwindBacktestRunner(BacktestRunner):
    """Backtests crowded_unwind long/short only, outside the live path."""

    def __init__(self, connection: sqlite3.Connection, **kwargs: Any) -> None:
        super().__init__(connection, **kwargs)
        self.long_setup = CrowdedUnwindLong()
        self.short_setup = CrowdedUnwindShort()
        self.decision_records: list[CrowdedDecisionRecord] = []
        self._records_by_object_id: dict[int, CrowdedDecisionRecord] = {}
        self._records_by_signal_id: dict[str, CrowdedDecisionRecord] = {}
    ...
    def _record_decision(
        ...
        *,
        ...
        record = CrowdedDecisionRecord(
            ...
            funding_pct_60d=float(features.funding_pct_60d),
            funding_8h=float(features.funding_8h),
            oi_zscore_60d=float(features.oi_zscore_60d),
            oi_delta_pct=float(features.oi_delta_pct),
            force_order_rate_60s=float(features.force_order_rate_60s),
            force_order_spike=bool(features.force_order_spike),
...
def build_crowded_report(
    *,
    result: BacktestResult,
    records: list[CrowdedDecisionRecord],
    source_db_path: Path,
    start_date: str,
    end_date: str,
    ...
    captured = [record for record in closed if record.pnl_r is not None and record.pnl_r > 0]
    return {
        ...
        "decision_summary": {
            ...
            "liquidation_capture_rate": _safe_div(len(captured), len(closed)),
        },
...
def write_crowded_markdown_report(report: dict[str, Any], output_path: Path) -> None:
    perf = report["performance"]
    summary = report["decision_summary"]
    lines = [
        "# Crowded Unwind Validation Report",
        "",
        ...
        f"- Liquidation capture rate: {summary['liquidation_capture_rate']}",
        "",
        "## Per Regime",
        "",
...
def run_crowded_backtest(
    *,
    ...
    try:
        ...
        runner = CrowdedUnwindBacktestRunner(conn, settings=settings)
        result = runner.run(
            BacktestConfig(
        ...
        write_crowded_markdown_report(report, DEFAULT_MARKDOWN_PATH)
        return report
    finally:
        conn.close()
...
def _top_rejection_reasons(records: list[CrowdedDecisionRecord]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _record_to_json(record: CrowdedDecisionRecord) -> dict[str, Any]:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest crowded_unwind research setup.")
