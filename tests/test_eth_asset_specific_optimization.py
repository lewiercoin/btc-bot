...
from research_lab.eth_asset_specific_optimization import (
    EthOptGates,
    baseline_variant,
...
def test_predeclared_grid_is_fixed_and_contains_baseline_point() -> None:
    ...
    assert len(grid) == 3
    assert len(ids) == 3
    assert "ETH_OPT_D0.00649" in ids
...
def test_baseline_variant_uses_trial_00095_transfer_knobs() -> None:
    ...
    assert variant.variant_id == "ETH_BASELINE_FROZEN_TRIAL_00095"
...
def test_builder_verdict_blocks_when_any_oos_gate_fails() -> None:
    ...
    assert builder_verdict(gates, selected_variant_id="ETH_OPT") == "ETH_OPTIMIZATION_NO_PROMOTION"
    assert builder_verdict({}, selected_variant_id=None) == "ETH_OPTIMIZATION_FAILED_NO_TRAIN_CANDIDATE"


def test_eth_asset_specific_hypothesis_spec_is_valid() -> None:
    spec = load_hypothesis_spec(Path("research_lab/hypotheses/active/eth_asset_specific_optimization.json"))

    assert spec.hypothesis_id == "eth_asset_specific_optimization_v1"
