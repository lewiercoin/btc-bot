...
from research_lab.diagnostics.oanda_eurusd_m15_sweep_reclaim_feasibility_v1 import (
    Candle,
    DiagnosticConfig,
    build_cohorts,
    build_event,
...
def _config() -> DiagnosticConfig:
    return DiagnosticConfig(
        ...
        min_hits=3,
        min_sweep_depth_pct=0.00030,
        shallow_min_depth_pct=0.00010,
...
def test_build_event_enforces_entry_candidate_and_return_start() -> None:
    ...
    assert event.stop_reference_price < event.entry_price
    assert event.risk_pct > 0
...
def test_build_cohorts_includes_predefined_controls() -> None:
    config = replace(_config(), min_sweep_depth_pct=0.0200, shallow_min_depth_pct=0.0010)
    candles = _base_candles(120)
    candles[20] = _candle(20, open_=100.05, high=101.0, low=96.9, close=100.2)
