...
### M1: LEVEL_SCANNER_FOUNDATION_V1

**Status:** DONE  
**Builder:** Codex  
**Auditor:** Claude Code (Windsurf)  
**Decision date:** 2026-06-04  
**Commit:** `cfe7e8f` (remote: origin/deploy/multi-asset-paper-v1)  
**Plan:** `research_lab/blueprints/level_scanner_foundation_v1_milestone.md`  
**Report:** `research_lab/validation_report_level_scanner.md`  
**Audit:** `research_lab/AUDITS/` (compliance, per-deliverable, findings register)

**Context:** M1 diagnostic confirmed setup scarcity (97.9% `signal_engine_no_candidate`, 74.9% `no_sweep`, 21.8% `sweep_too_shallow`). Setup portfolio expansion approved. `level_scanner` is prerequisite factory for all future setup candidates.

**Scope:** Research-only module `research_lab/level_scanner.py` with 26-column parquet schema. Six level categories: session extremes, PDH/PDL/PWH/PWL, EQH/EQL clusters, anchored VWAP, round numbers, liquidation placeholder. Deterministic `level_id` SHA256 hash (F014 fix). Explicit empty liquidation stub (F015 fix). Layer separation enforced (no signal/governance/risk imports).

**Validation:**
- Focused unit tests: 9/9 passed
- Compileall: exit code 0
- 6m BTCUSDT smoke: 9,230 levels emitted, 0 duplicate `level_id`
- SMC cross-validation: session boundaries documented (`[start,end)` vs end-inclusive), PDH/PDL values match

**Audit verdict:** MVP_DONE (3 minor advisory recommendations, non-blocking)
...
**Audit:** Claude Code PASSED (1eb9c2d). Layer separation, contract compliance,
determinism, backward compatibility, state integrity, test coverage — all PASS.

**Deployed to production:** 2026-05-22 20:40 UTC. Verified ETH/SOL FeatureEngine
bootstrap with OI/CVD history. No fallback warnings.
...
separate gated multi-symbol PAPER decision loop, explicit multi-asset state
tables/recovery helpers, and per-call paper execution symbol routing. It does
not enable ETH/SOL PAPER, change production settings, deploy to production,
change M4, or alter BTC-only behavior when `multi_asset.enabled=false`.

**Implementation:**
- Added `_run_multi_asset_paper_decision_cycle()` behind
  `settings.mode == PAPER`, `multi_asset.enabled=true`, and more than one
  enabled symbol.
- Kept the existing single-symbol BTC decision cycle as the default path.
- Added explicit `StateStore.ensure_multi_asset_schema()` for `symbol_state`
  and `portfolio_state`. The normal startup migration path does not create
  these tables while multi-asset mode is disabled.
- Added portfolio recovery helpers that derive open positions and recent trade
  state from `positions`/`trade_log`, then overlay persisted pause/emergency
  state.
- Added per-symbol lifecycle filtering and REST-only non-BTC snapshot assembly
  in the multi-symbol loop to avoid reusing BTC websocket buffers for ETH/SOL.
- Added per-call symbol routing to paper execution while preserving default
  BTC-only constructor behavior. Live execution rejects symbol mismatches.

**Boundaries:**
- Production remains `multi_asset.enabled=false`; ETH/SOL activation requires a
  separate approval milestone.
- No production deploy is included in this milestone.
- No production DB migration runs while multi-asset mode is disabled.
- No change to the deployed BTC PAPER strategy parameters or shadow thresholds.
- No live multi-symbol execution path is activated.

**Validation:**
- `pytest tests/test_multi_asset_state_store.py tests/test_multi_asset_orchestrator_dispatch.py tests/test_execution_symbol_gates.py -q -o addopts=` -> 8 passed.
- `pytest tests/test_settings.py tests/test_core_portfolio_gate.py tests/test_portfolio_state.py tests/test_execution_symbol_gates.py tests/test_paper_execution_realism.py tests/test_shadow_real_signal_cycle.py tests/test_sidecar_isolation.py tests/test_shadow_schema.py tests/test_multi_asset_state_store.py tests/test_multi_asset_orchestrator_dispatch.py -q -o addopts=` -> 58 passed.
...
**Status:** DEPLOYED - dormant runtime contracts active on production
**Builder:** Codex
**Decision date:** 2026-05-21
**Deployment date:** 2026-05-21
**Branch:** `deploy/multi-asset-paper-v1`
**Contract review:** `docs/MULTI_ASSET_PAPER_RUNTIME_CONTRACT_REVIEW_V1.md`
**Audit:** `docs/audits/AUDIT_MULTI_ASSET_PAPER_RUNTIME_FOUNDATION_V1_2026-05-21.md` (DONE)
...
**Deployment verification:** Production reached commit `8f5e7c94` with
`multi_asset.enabled=false`, `enabled_symbols=("BTCUSDT",)`, BTC PAPER active,
ETH paper execution blocked before writes, and no `symbol_state` table created.

**Next:** `MULTI_ASSET_PAPER_ORCHESTRATOR_LOOP_V1` implements the dormant
multi-symbol loop and state persistence contracts. ETH/SOL PAPER activation
remains a separate approval milestone.
...
**Scope:** Shadow-sidecar-only SOL threshold update. SOL
`min_sweep_depth_pct` changes from the frozen BTC transfer value `0.00649` to
the audited SOL asset-specific candidate `0.0075`.

**Boundaries:**
- BTC remains at frozen `trial-00095` threshold `0.00649`.
- ETH remains at audited shadow threshold `0.0075`.
- SOL remains `shadow_no_orders`; this does not approve PAPER or LIVE.
...
**Evidence:**
- SOL optimization selected `SOL_OPT_D0.00750`: OOS ER 2.573 vs 2.041
  baseline, PF 4.29 vs 3.32, DD 3.57% vs 7.92%.
- Portfolio diagnostic candidate profile BTC 0.00649 / ETH 0.0075 /
  SOL 0.0075: 267 portfolio trades, ER 2.364, PF 4.38, max DD 8.77R.
- Candidate vs current shadow: -36 portfolio trades (-11.9%), ER +15.4%,
  PF +18.3%, DD -46.8%, SOL retention 73.2%.

**Validation:**
- `tests/test_shadow_real_signal_cycle.py` verifies BTC uses `0.00649` while
  ETH/SOL use `0.0075`.
...
**Audit:** `docs/audits/AUDIT_DEPTH_THRESHOLD_PORTFOLIO_IMPACT_DIAGNOSTIC_V1_2026-05-21.md`
...
**Next:** `SOL_SHADOW_DEPTH_PARAMETER_UPDATE_V1` if user chooses incremental
shadow parameter migration. This diagnostic does not approve PAPER, LIVE, or
runtime.
...
Vision ZIPs, discards raw archives, enforces a free-disk guard before each day,
and records resumable per-day checkpoints. It does not write to production
...
**Status:** DONE / **Audit:** `docs/audits/AUDIT_OPTUNA_INFRASTRUCTURE_V3_HARDENING_2026-05-07.md`  
**Builder:** Codex  
...
**Deliverables:**
1. Raw vs objective metrics separation: `raw_metrics_json`, `objective_metrics_json` columns; audit trail for TPE transforms
2. WF-winners-only warm-start: `--warm-start-mode={all|wf-winners-only}` with production default `wf-winners-only`
3. Multivariate TPE auto-disable: prevents RandomSampler fallback for dynamic-bound params (ema_slow, tp2_atr_mult, high_vol_leverage)
4. Richer trial metadata: 14 new Optuna trial attrs (protocol, search space, date range, sampler policy, objective metrics, wall time)
5. V3 range tightening: 7 params tightened based on V2 accepted/rejected evidence (documented in `OPTUNA_V3_RANGE_TIGHTENING_2026-05-07.md`)
6. Drawdown unit alignment: `max_drawdown_pct_per_window: 0.5` = 50% (fraction units)

**Validation:**
- Research Lab smoke: 40/42 passed (2 intentional skips for 8f2c6f2 baseline)
- Full suite: 311/335 passed, 24 skipped
- `compileall research_lab tests/test_research_lab_smoke.py`: PASS
- 10-trial synthetic Optuna smoke campaign: PASS (multivariate_tpe_effective=false, policy documented, raw/objective metrics persisted)

**Audit verdict:** DONE (all axes PASS, tech debt LOW, no blockers for Campaign V3 launch)
