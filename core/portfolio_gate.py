...
class RuntimePortfolioGate:
    """Deterministic runtime-safe portfolio gate for candidate signal batches."""
...
class ResearchPortfolioGate(RuntimePortfolioGate):
    """Backward-compatible alias for existing research milestones."""
...
def _directional_notional_after(
    ...
    accepted_notional = sum(s.gross_notional_pct for s in accepted if s.normalized_direction == signal.normalized_direction)
    if signal.normalized_direction == "LONG":
        return portfolio_state.directional_notional_pct_long + accepted_notional + signal.gross_notional_pct
    return portfolio_state.directional_notional_pct_short + accepted_notional + signal.gross_notional_pct
...
def _same_utc_day(value: datetime, now: datetime) -> bool:
    return _to_utc(value).date() == _to_utc(now).date()


def _within_rolling_days(value: datetime, now: datetime, *, days: int) -> bool:
    return _to_utc(value) >= _to_utc(now) - timedelta(days=days)
